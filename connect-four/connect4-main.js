...
function winCondition(number, player) {
	...
	switch (player) {
		case 0:
			...
			tempArray.pop();
			tempArray = tempArray.map(function (values) {
			...
			if (player1.size > 3) {
				for (let i = 0; i < 4; i++) {
					...
					if (
						winPlayer1(horiz) ||
						winPlayer1(diagnallUp) ||
						winPlayer1(diagnalDown) ||
						winPlayer1(vertical)
					) {
						win('player1');
		...
		case 1:
			...
			tempArray1.pop();
			tempArray1 = tempArray1.map(function (values) {
				return Number(values.id);
			});
			...
			if (player2.size > 3) {
				for (let i = 0; i < 4; i++) {
					...
					if (
						winPlayer2(horiz) ||
						winPlayer2(diagnallUp) ||
						winPlayer2(diagnalDown) ||
						winPlayer2(vertical)
					) {
						win('player2');
					} else if (player === boardSize[0] * boardSize[1]) {
